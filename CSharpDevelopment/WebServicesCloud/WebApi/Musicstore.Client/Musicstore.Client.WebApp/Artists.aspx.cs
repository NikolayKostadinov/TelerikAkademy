...
namespace Musicstore.Client.WebApp
{
    public partial class Artists : System.Web.UI.Page
    {
        ...
        protected void btnArtistSave_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtArtistName.Text))
            {
                ...
                if (!string.IsNullOrEmpty(btnArtistSave.CommandArgument))
                {
                    ...
                    var response = SessionState.Client.PutAsJsonAsync("api/artist/" + artist.Id, artist).Result;
                    if (response.IsSuccessStatusCode)
                    {
                ...
                {
                    var response = SessionState.Client.PostAsJsonAsync("api/artist", artist).Result;
                    if (response.IsSuccessStatusCode)
                    {
                        var a = response.Content.ReadAsAsync<HttpResponseMessage>().Result;
                    }
        ...
        {
            GridViewRow row = grdArtists.SelectedRow;
            btnArtistSave.CommandArgument = row.Cells[1].Text;
            txtArtistName.Text = row.Cells[2].Text;
            txtArtistCountry.Text = row.Cells[3].Text;
        }
