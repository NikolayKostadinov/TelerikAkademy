    ...
    public class FigureControllerEx : FigureController
    {
        public override void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                ...
                case "cylinder":
                    {
                        Vector3D bottomCircle = Vector3D.Parse(splitFigString[1]);
                        Vector3D topCircle = Vector3D.Parse(splitFigString[2]);
                        double radius = double.Parse(splitFigString[3]);
                        this.currentFigure = new Cylinder(bottomCircle, topCircle, radius);
                        break;
                    }
                default:
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        IAreaMeasurable figArea = this.currentFigure as IAreaMeasurable;
                        if (figArea == null)
                        {
                        ...
                        {
                            Console.WriteLine("{0:0.00}", figArea.GetArea());
                        }
                        break;
                    }
                case "volume":
                    {
                        IVolumeMeasurable figVolume = this.currentFigure as IVolumeMeasurable;
                        if (figVolume == null)
                        {
                            Console.WriteLine("undefined");
                        }
                        ...
                        {
                            Console.WriteLine("{0:0.00}", figVolume.GetVolume());
                        }
                        break;
                    }
                case "normal":
                    {
                        IFlat figFlat = this.currentFigure as IFlat;
                        if (figFlat == null)
                        {
                            Console.WriteLine("undefined");
                        }
                        ...
                        {
                            Console.WriteLine("{0:0.00}", figFlat.GetNormal());
                        }
                        break;
                    }
                default:
